export class Camera {
  ...
  public readonly verticalGroundOffset: number = 150;
  ...
  public setPositionImmediate(targetX: number, targetY: number): void {
    ...
    this.y = targetY - this.height * 0.55;
    this.clampToBounds();
  }
  ...
  public update(dt: number, focusX: number, focusY: number, facingRight: boolean, vx: number = 0): void {
    ...
    // Focus a 0.55 per mantenere l'asse del terreno e la porzione sotto i piedi ben visibile ed equilibrata.
    const desiredY = focusY - this.height * 0.55;
